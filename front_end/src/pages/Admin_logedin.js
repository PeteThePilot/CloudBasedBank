...
function Admin_Logedin() {
        ...
        try {
            ...
            switch (typeofuser) {
                case "cashier":
                    ...
                    break;
                case "admin":
                    if (parseFloat(depositAmountToday) + parseFloat(amountDeposit) > 100000) {
                        alert("Too much money deposited today, try again tomorrow");
                        navigate("/admin");
                        return; 
                    }
    ...
    const handleDeleteUser = async (type) => {
        try {
            const token = sessionStorage.getItem('token');
            const endpoint = type === 'cashier' ? 'delete-cashier' : 'delete-user-account';

            const response = await axios.delete(`https://localhost:3001/${endpoint}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                },
            ...
            alert(`An error occurred while deleting ${type}. Please try again later.`);
        }
    };
    ...
    const handleSubmitTransactions = async () => {
        try {
            ...
            const response = await axios.get('https://localhost:3001/transactions', {
                params: { account_from: accountFrom },
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
            ...
            alert('An error occurred while fetching the transactions. Please try again later.');
        }
    };
        ...
        <div style={{ backgroundColor: '#98c1d9', minHeight: '100vh', padding: '20px' }}>
            ...
            <div>
                ...
                <Button
                    variant="contained"
                    color="secondary"
                    onClick={() => handleDeleteUser('cashier')}
                    style={{ marginTop: '20px', backgroundColor: 'red' }}
                >
                    ...
                    <div style={{ marginTop: '20px' }}>
                        ...
                        {transactions.map((transaction) => (
                            <div key={transaction._id}>
                                <Typography>Account From: {transaction.account_from}</Typography>
                                <Typography>Account To: {transaction.account_to}</Typography>
                                <Typography>Money: {transaction.money}</Typography>
                                <Typography>Username: {transaction.username}</Typography>
                                <Typography>Type of User: {transaction.typeofuser}</Typography>
                                <hr />
                            </div>
                        ))}
                    </div>
